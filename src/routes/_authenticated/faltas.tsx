          ...
          widgets={[
            ...
            ...(columnsDetected
              ? [
                  {
                    key: "gerentes",
                    titulo: "Faltas por Gerente de Área",
                    tamanho: "grande" as const,
                    conteudo: (
                      <section className="space-y-3">
                        <h2 className="text-base font-semibold text-foreground">
                          Faltas por Gerente de Área
                        </h2>
                        <p className="text-xs text-muted-foreground">
                          Clique em um supervisor para ver as faltas que ele lançou.
                        </p>
                        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                          {gerenteFaltasCards.map((g, idx) => {
                            const color = CARD_COLORS[idx % CARD_COLORS.length]!;
                            const shortName = gerenteAreaACanonico(g.name) ?? g.name;
                            const aberto = gerenteAberto === g.name;
                            return (
                              <button
                                key={g.name}
                                type="button"
                                onClick={() => setGerenteAberto(aberto ? null : g.name)}
                                aria-expanded={aberto}
                                className={`rounded-xl border p-4 text-left transition-all hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-primary/50 ${
                                  aberto
                        ...
                        {gerenteAberto && (
                          <div className="rounded-xl border border-border bg-card p-4">
                            {(() => {
                              const g = gerenteFaltasCards.find((x) => x.name === gerenteAberto);
                              if (!g) return null;
                              const nomeCompleto = gerenteAreaACanonico(g.name) ?? g.name;
                              return (
                                <>
                                  <h3 className="text-sm font-semibold text-foreground">
                                    Faltas lançadas por {nomeCompleto}
                                  </h3>
                                  {g.detalhes.length === 0 ? (
                                    <p className="mt-3 text-sm text-muted-foreground">
                                      Nenhuma falta lançada por este supervisor no período/filtros
                                      atuais.
                                    </p>
                                  ) : (
