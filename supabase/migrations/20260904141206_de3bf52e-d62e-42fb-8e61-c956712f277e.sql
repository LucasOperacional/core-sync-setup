...
create policy "Users can insert own conversations" on public.ai_conversations for insert to authenticated with check (auth.uid() = user_id);
create policy "Users can update own conversations" on public.ai_conversations for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy "Users can view own messages" on public.ai_messages for select to authenticated using (auth.uid() = user_id);
create policy "Users can insert own messages" on public.ai_messages for insert to authenticated with check (auth.uid() = user_id);
create policy "Users can update own messages" on public.ai_messages for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "Users can delete own messages" on public.ai_messages for delete to authenticated using (auth.uid() = user_id);
...
grant execute on function public.is_room_admin(uuid, uuid) to service_role;
...
create policy "Room admins can update rooms" on public.chat_rooms for update to authenticated
  using (public.is_room_admin(id, auth.uid())) with check (public.is_room_admin(id, auth.uid()));

create policy "Members can view room members" on public.chat_room_members for select to authenticated
  using (public.is_room_member(room_id, auth.uid()));
create policy "Authenticated users can insert members" on public.chat_room_members for insert to authenticated with check (auth.uid() is not null);
create policy "Members can remove themselves" on public.chat_room_members for delete to authenticated using (user_id = auth.uid());
...
create policy "Users can upsert own reads" on public.chat_message_reads for insert to authenticated with check (auth.uid() = user_id);
create policy "Users can update own reads" on public.chat_message_reads for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);

create policy "Authenticated users can upload chat attachments" on storage.objects for insert to authenticated
  with check (bucket_id = 'chat-attachments' and auth.uid() is not null);
create policy "Authenticated users can read chat attachments" on storage.objects for select to authenticated
...
CREATE POLICY "admin_full_access_ia_training"
  ON public.ia_training_data FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "authenticated_select_ia_training" ON public.ia_training_data FOR SELECT TO authenticated USING (auth.uid() IS NOT NULL);
CREATE POLICY "authenticated_insert_ia_training" ON public.ia_training_data FOR INSERT TO authenticated WITH CHECK (auth.uid() IS NOT NULL AND auth.uid() = user_id);
CREATE POLICY "owner_update_ia_training" ON public.ia_training_data FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "owner_delete_ia_training" ON public.ia_training_data FOR DELETE TO authenticated USING (auth.uid() = user_id);
...
create policy "Authenticated users can view all profiles" on public.user_profiles for select to authenticated using (auth.uid() is not null);
