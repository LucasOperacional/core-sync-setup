...
CREATE POLICY "lgpd_consents_own_read" ON public.lgpd_consents FOR SELECT TO authenticated USING (user_id = auth.uid() OR public.has_role(auth.uid(), 'admin'));
CREATE POLICY "lgpd_consents_own_insert" ON public.lgpd_consents FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
...
DROP TRIGGER IF EXISTS lgpd_solicitacoes_updated_at ON public.lgpd_solicitacoes;
...
DROP TRIGGER IF EXISTS lgpd_incidentes_updated_at ON public.lgpd_incidentes;
...
CREATE POLICY "lgpd_acessos_read" ON public.lgpd_acessos FOR SELECT TO authenticated USING (user_id = auth.uid() OR public.has_role(auth.uid(), 'admin'));
CREATE POLICY "lgpd_acessos_insert" ON public.lgpd_acessos FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());