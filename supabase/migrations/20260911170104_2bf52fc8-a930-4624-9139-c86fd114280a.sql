...
create policy "Admins can insert queues" on public.chat_queues for insert to authenticated with check (private.has_role(auth.uid(),'admin'::public.app_role));
create policy "Admins can update queues" on public.chat_queues for update to authenticated using (private.has_role(auth.uid(),'admin'::public.app_role));
create policy "Admins can delete queues" on public.chat_queues for delete to authenticated using (private.has_role(auth.uid(),'admin'::public.app_role));
...
create policy "Admins can insert queue agents" on public.chat_queue_agents for insert to authenticated with check (private.has_role(auth.uid(),'admin'::public.app_role));
create policy "Admins can delete queue agents" on public.chat_queue_agents for delete to authenticated using (private.has_role(auth.uid(),'admin'::public.app_role));
...
CREATE POLICY folhas_pdf_select ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'folhas-pdf' AND private.owns_protocolo_path(name));
DROP POLICY IF EXISTS folhas_pdf_insert ON storage.objects;
CREATE POLICY folhas_pdf_insert ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'folhas-pdf' AND private.owns_protocolo_path(name));
DROP POLICY IF EXISTS folhas_pdf_update ON storage.objects;
CREATE POLICY folhas_pdf_update ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'folhas-pdf' AND private.owns_protocolo_path(name)) WITH CHECK (bucket_id = 'folhas-pdf' AND private.owns_protocolo_path(name));
DROP POLICY IF EXISTS folhas_pdf_delete ON storage.objects;
CREATE POLICY folhas_pdf_delete ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'folhas-pdf' AND private.owns_protocolo_path(name));

DROP POLICY IF EXISTS "Admins leem arquivos dos dashboards" ON storage.objects;
CREATE POLICY "Admins leem arquivos dos dashboards" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'arquivos-dashboards' AND private.has_role(auth.uid(), 'admin'::public.app_role));
DROP POLICY IF EXISTS "Admins enviam arquivos dos dashboards" ON storage.objects;
CREATE POLICY "Admins enviam arquivos dos dashboards" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'arquivos-dashboards' AND private.has_role(auth.uid(), 'admin'::public.app_role));
DROP POLICY IF EXISTS "Admins atualizam arquivos dos dashboards" ON storage.objects;
CREATE POLICY "Admins atualizam arquivos dos dashboards" ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'arquivos-dashboards' AND private.has_role(auth.uid(), 'admin'::public.app_role)) WITH CHECK (bucket_id = 'arquivos-dashboards' AND private.has_role(auth.uid(), 'admin'::public.app_role));
DROP POLICY IF EXISTS "Admins removem arquivos dos dashboards" ON storage.objects;
CREATE POLICY "Admins removem arquivos dos dashboards" ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'arquivos-dashboards' AND private.has_role(auth.uid(), 'admin'::public.app_role));
...
CREATE POLICY "Admins manage nexti config" ON public.nexti_config FOR ALL TO authenticated USING (private.has_role(auth.uid(), 'admin'::public.app_role)) WITH CHECK (private.has_role(auth.uid(), 'admin'::public.app_role));
